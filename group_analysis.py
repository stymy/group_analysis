...
#  mask_file = 'mask_dparsf.nii' #standard
  mask_file = 'mask_%s_%s.nii.gz' %(pipeline, strategy)
  if not os.path.exists(mask_file):
      path_list = get_s3_paths.get_paths(sub_pheno_list, pipeline, strategy, 'func_mask', download_root)
      get_s3_paths.download(path_list, download_root, s3_prefix)
      maskdir = os.path.join(download_root, pipeline, strategy, "func_mask")
      shutil.copyfile("mask.sh", os.path.join(maskdir,"mask.sh"))
      os.chdir(maskdir)
      subprocess.call(["sh", "mask.sh"])
      os.chdir(currentdir)
      shutil.copyfile(os.path.join(maskdir,"ninety_mask.nii.gz"), mask_file)
